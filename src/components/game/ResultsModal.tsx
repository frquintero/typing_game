...
export const ResultsModal: React.FC<ResultsModalProps> = React.memo(({ wpm, accuracy, errors, focusMode, onRetry }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 id="results-stats-heading" className="sr-only">Game Statistics</h3>
          {!focusMode && (
            <motion.div
        ...
        >
          <motion.div
            ...
            className="flex-1"
          >
            <Button onClick={onRetry} className="w-full min-h-[48px] touch-manipulation text-base font-semibold" aria-describedby="retry-button-description">
              Try Again
            </Button>
            <span id="retry-button-description" className="sr-only">Start a new typing test with the same settings</span>
          </motion.div>
        </motion.div>
      </motion.div>
    </motion.div>
